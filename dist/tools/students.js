import { z } from "zod";
export function registerStudentTools(server, canvas) {
    // Tool: list-students
    server.tool("list-students", "Get a complete list of all students enrolled in a specific course", {
    ...
    }, async ({ courseId, includeEmail, anonymous = true }) => {
        const students = [];
        let page = 1;
        let hasMore = true;
        try {
            while (hasMore) {
                ...
                const pageStudents = await canvas.listStudents(courseId, params, { anonymous });
