import { z } from "zod";
export function registerRubricTools(server, canvas) {
    // Tool: list-rubrics
    // server.tool(
    //   "list-rubrics",
    ...
    server.tool("list-rubric-assessments", "List all rubric assessments for an assignment.", {
        courseId: z.string().describe("The ID of the course"),
        assignmentId: z.string().describe("The ID of the assignment"),
        ...
        try {
            const rubricAssessments = await canvas.listRubricAssessments(courseId, assignmentId, { 'include[]': 'rubric_assessment' }, { anonymous });
            return {
                content: [
                    {
                        type: "text",
