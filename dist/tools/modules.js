import { z } from "zod";
export function registerModuleTools(server, canvas) {
    // Tool: list-modules
    server.tool("list-modules", "Return all modules in a course (optionally inline items).", {
        ...
        try {
            while (hasMore) {
                ...
                const pageModules = await canvas.listModules(courseId, params);
    ...
    server.tool("list-module-items", "Given a module ID, list its items (pages, quizzes, files, etc).", {
        courseId: z.string().describe("The ID of the course"),
        ...
        try {
            while (hasMore) {
                ...
                const pageItems = await canvas.listModuleItems(courseId, moduleId, params);
