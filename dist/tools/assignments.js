import { z } from "zod";
export function registerAssignmentTools(server, canvas) {
    // Tool: list-assignments
    server.tool("list-assignments", "Get a list of all assignments in a course with submission status for students", {
    ...
    }, async ({ courseId, studentId, includeSubmissionHistory = false, anonymous = true }) => {
        let assignments = [];
        let page = 1;
        let hasMore = true;
        try {
            while (hasMore) {
                ...
                const pageAssignments = await canvas.listCourseAssignments(courseId, params, { anonymous });
    ...
    // Tool: get-assignment
    server.tool("get-assignment", "Fetch metadata for a single assignment (due date, points, rubric, submission types, etc).", {
        courseId: z.string().describe("The ID of the course"),
